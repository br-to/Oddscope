...
- **Global scope**: `/Users/toikobara/Oddscope/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/toikobara/Oddscope/.claude/notes/`)
...
2. Glob `/Users/toikobara/Oddscope/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/toikobara/Oddscope/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/toikobara/Oddscope/.claude/notes/` — works in any directory
